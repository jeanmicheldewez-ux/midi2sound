...
// Minimal public facade for standalone sound-engine demos.
// Existing Ultragear code keeps using the original globals and functions above.
window.UltraSoundEngine = {
    async start() {
        flagSoundOn = true;
        const soundToggle = document.getElementById("soundon");
        if (soundToggle) soundToggle.checked = true;
        const soundPanel = document.getElementById("sound");
        if (soundPanel) soundPanel.style.display = "";
        if (!soundInit) await initTone();
        return this.getState();
    },

    setEnabled(enabled) {
        flipSoundOn(Boolean(enabled));
        return this.getState();
    },

    ensureTracks(count = 1, synthTypes = ['Synth']) {
        const wanted = Math.max(0, Number(count) || 0);
        while (tracks.length < wanted) {
            const nextIndex = tracks.length;
            actualTrack = nextIndex;
            flagChangeTone = false;
            plusTrack();
            addOsc(synthTypes[nextIndex] || synthTypes[0] || 'Synth');
        }
        return this.getTracks();
    },

    createTrack(synthType = 'Synth') {
        const nextIndex = tracks.length;
        actualTrack = nextIndex;
        flagChangeTone = false;
        plusTrack();
    ...
    setActiveTrack(index) {
        ...
        actualTrack = nextIndex;
        return this.getTrack(nextIndex);
    },
    ...
    getTrack(index) {
        ...
        return {
            index: Number(index),
            tone: track.tones ? track.tones.name : null,
            effects: track.fx ? track.fx.map(effect => effect.name) : [],
            volume: track.vol && track.vol.values ? track.vol.values[0] : null,
            sample: track.sample || null
        };
    },

    getTracks() {
        return tracks.map((_, index) => this.getTrack(index));
    },

    noteOn(noteOrMidi, trackIndex = actualTrack) {
        const note = typeof noteOrMidi === 'number' ? getNoteName(noteOrMidi) : noteOrMidi;
        noteOn(note, Number(trackIndex));
    },

    noteOff(noteOrMidi, trackIndex = actualTrack) {
        const note = typeof noteOrMidi === 'number' ? getNoteName(noteOrMidi) : noteOrMidi;
        noteOff(note, Number(trackIndex));
    },

    handleMidiMessage(status, data1, data2) {
        midiSound(status, data1, data2);
    },

    setMasterVolume(db) {
        if (Tone && Tone.Destination && Tone.Destination.volume) {
            Tone.Destination.volume.value = Number(db);
        }
    },

    setTrackVolume(trackIndex, db) {
        const track = tracks[Number(trackIndex)];
        if (!track || !track.tones) return false;
        const value = Number(db);
        track.tones.set({ volume: value });
        if (track.vol && typeof track.vol.set === 'function') {
            track.vol.set({ values: [value] });
        }
        return true;
    },

    exportTemplate() {
        return tracks.map(track => extractTrackParams(track));
    },

    importTemplate(template) {
        if (!Array.isArray(template)) {
            throw new Error('Sound template must be an array of tracks.');
        }
        emptySounds();
        fillSounds(template);
        flipSoundOn(true);
    ...
    getState() {
        return {
            soundInit,
            soundEnabled: flagSoundOn,
            activeTrack: actualTrack,
            trackCount: tracks.length
        };
    }
};
